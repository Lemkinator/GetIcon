...
On emulators, `BaselineProfileGenerator` sets `debug.hwui.renderer=skiavk`. Under GLES, the
emulator's SwiftShader translator crashes on the app list's hardware layers. Gradle then reports
only `Test failed with status -1`.
